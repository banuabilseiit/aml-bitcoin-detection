...
with left_col:
    ...
    feat_5  = st.slider("local_feat_5  — основной параметр",  -10.0, 10.0, 0.0, 0.1)
    feat_53 = st.slider("local_feat_53 — параметр суммы",      0.0, 100.0, 10.0, 0.5)
    feat_59 = st.slider("local_feat_59 — параметр комиссии",   0.0, 50.0, 1.0, 0.1)
    feat_2  = st.slider("local_feat_2  — входы транзакции",    1.0, 50.0, 2.0, 1.0)
    feat_55 = st.slider("local_feat_55 — параметр выходов",    0.0, 50.0, 5.0, 0.5)

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("**Агрегированные параметры**")
    agg_32  = st.slider("agg_feat_32 — агрегированный 1",     -5.0, 5.0, 0.0, 0.1)
    agg_49  = st.slider("agg_feat_49 — агрегированный 2",     -5.0, 5.0, 0.0, 0.1)
